...
mmapGpio::mmapGpio(){
	...
	// Initialize the mutex for the access to the resource
	pthread_mutex_init(&access_mux, nullptr);
}
...
void mmapGpio::setPinDir(unsigned int pinnum, const unsigned int &dir){
	...
	pthread_mutex_unlock(&access_mux);
}
...
unsigned int mmapGpio::readPin(unsigned int pinnum){
	...
	condition = (*(this->gpio + GPFLEV0) & (1 << pinnum)) != 0;
	if (condition)
		retVal = 1;
